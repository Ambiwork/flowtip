...
function getValidRegions(config) {
  ...
  var offsetBounds = Rect.grow(bounds, -edgeOffset); // This value is true if `overlap` amount of the target rect intersects
...
function getExternalRegion(config) {
  var target = config.target,
      ...
      edgeOffset = config.edgeOffset,
      disabled = config.disabled;
  var offsetBounds = Rect.grow(bounds, -edgeOffset);
...
function flowtip(config) {
  ...
  var intersect = Rect.intersect(finalConfig.target, rect);
